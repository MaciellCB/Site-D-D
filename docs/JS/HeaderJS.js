...
// RECUPERAÇÃO DE INICIATIVA SALVA (Refresh)
window.addEventListener('sheet-updated', () => {
...
/* =============================================================
   SISTEMA DE MAPA V2.0 (INTERATIVO, CONTEXTO, SIDEBAR)
============================================================= */
let mapInstance = null;
let currentPlane = 'material';
let tempClickCoords = null; // Guarda onde clicou com botão direito
let isMeasuring = false;
let measurePoints = [];
let measureLine = null;
let mapMarkers = []; // Array para guardar os objetos markers e filtrar depois

// BANCO DE DADOS DOS PLANOS (Imagens)
// Coloque os arquivos na pasta img/imagem-no-site/
const PLANES_DB = {
    'material': { img: 'img/imagem-no-site/mapa-runeterra.jpg', name: 'Material' },
    'shadowfell': { img: 'img/imagem-no-site/mapa-shadowfell.jpg', name: 'Pendor das Sombras' },
    'feywild': { img: 'img/imagem-no-site/mapa-feywild.jpg', name: 'Agréstia das Fadas' },
    'astral': { img: 'img/imagem-no-site/mapa-astral.jpg', name: 'Plano Astral' }
};
...
function abrirMapaMundi() {
    ...
    overlay.style.display = 'block'; // Block para ocupar tudo
...
function initLeafletMap() {
    ...
    mapInstance = L.map('leaflet-map-container', {
        ...
        minZoom: -3,
        maxZoom: 3,
        zoomControl: false, // Vamos reposicionar ou usar scroll
        attributionControl: false
    });

    // Carrega o plano inicial
    carregarImagemPlano('material');

    // Evento: Botão Direito (Context Menu)
    mapInstance.on('contextmenu', function(e) {
        tempClickCoords = e.latlng;
        
        const menu = document.getElementById('map-context-menu');
        // Posiciona onde o mouse está (usando containerPoint para pixel relativo à div)
    ...
    // Evento: Clique Esquerdo (Fecha menu e Medição)
    mapInstance.on('click', function(e) {
        document.getElementById('map-context-menu').style.display = 'none';
        
        if (isMeasuring) {
            processarMedicao(e.latlng);
        }
    });

    // Evento: Zoom/Move (Fecha menu)
    mapInstance.on('movestart', () => document.getElementById('map-context-menu').style.display = 'none');
    mapInstance.on('zoomstart', () => document.getElementById('map-context-menu').style.display = 'none');

    // Adiciona Pins Iniciais (Exemplo)
    criarPino(700, 1100, "Noxus", "Império militar.", null);
    criarPino(650, 450, "Demacia", "Reino da justiça.", null);
}

// 3. SISTEMA DE PLANOS
let currentLayer = null;

function carregarImagemPlano(planoKey) {
    if (currentLayer) mapInstance.removeLayer(currentLayer);
    
    // Fallback de imagem se não existir
    const url = PLANES_DB[planoKey] ? PLANES_DB[planoKey].img : 'img/imagem-no-site/dado.png';
    
    // Define limites (ajuste conforme o tamanho real da sua imagem)
    const w = 1920; 
    const h = 1080;
    const bounds = [[0, 0], [h, w]];

    currentLayer = L.imageOverlay(url, bounds).addTo(mapInstance);
    mapInstance.fitBounds(bounds);
    currentPlane = planoKey;
}

function mudarPlano(val) {
    // Aqui você poderia salvar os pins do plano atual e carregar os do novo
    // Por enquanto, apenas troca a imagem
    carregarImagemPlano(val);
}

// 4. CRIAR PINS (POPUP E LÓGICA)
function abrirModalCriarPin() {
    document.getElementById('map-context-menu').style.display = 'none';
    document.getElementById('modal-criar-pin').style.display = 'flex';
    
    // Limpa campos
    document.getElementById('pin-nome').value = "";
    document.getElementById('pin-desc').value = "";
    document.getElementById('pin-img-preview').src = "";
    document.getElementById('pin-img-preview').style.display = 'none';
    document.getElementById('pin-paste-text').style.display = 'block';
    
    // Foca na área de paste
    setTimeout(() => document.getElementById('pin-paste-area').focus(), 100);
}

function fecharModalCriarPin() {
    document.getElementById('modal-criar-pin').style.display = 'none';
}

// Lógica de Colar Imagem (Paste)
const pasteArea = document.getElementById('pin-paste-area');
const fileInputMap = document.getElementById('pin-file-input');
const previewMap = document.getElementById('pin-img-preview');
const textMap = document.getElementById('pin-paste-text');

pasteArea.onclick = () => fileInputMap.click();

fileInputMap.onchange = (e) => {
...
function renderPreview(file) {
    const reader = new FileReader();
    reader.onload = (e) => {
        previewMap.src = e.target.result;
        previewMap.style.display = 'block';
        textMap.style.display = 'none';
    };
    reader.readAsDataURL(file);
}

function salvarNovoPin() {
    const nome = document.getElementById('pin-nome').value;
    const desc = document.getElementById('pin-desc').value;
    const img = previewMap.style.display === 'block' ? previewMap.src : null;

    if (!nome) { alert("Dê um nome ao local!"); return; }

    // Cria o pino nas coordenadas salvas do clique direito
    if (tempClickCoords) {
        criarPino(tempClickCoords.lat, tempClickCoords.lng, nome, desc, img);
    }

    fecharModalCriarPin();
}

function criarPino(lat, lng, titulo, desc, imgSrc) {
    // Ícone Customizado (Opcional - usa padrão se não definir)
    // const icon = L.divIcon({className: 'custom-pin', html: '📍'}); 

    const marker = L.marker([lat, lng]).addTo(mapInstance);
    
    let popupContent = `<div style="text-align:center;">`;
    if (imgSrc) {
        popupContent += `<img src="${imgSrc}" style="width:100%; max-height:150px; object-fit:cover; border-radius:4px; margin-bottom:8px; cursor:pointer;" onclick="window.openImageLightbox('${imgSrc}')">`;
    }
    popupContent += `<h3>${titulo}</h3><p>${desc || ''}</p></div>`;

    marker.bindPopup(popupContent);

    // Salva na lista para busca
    const pinData = { marker, titulo, desc, lat, lng };
    mapMarkers.push(pinData);
    
    atualizarListaSidebar();
}

// 5. SIDEBAR: BUSCA E LISTA
function atualizarListaSidebar() {
    const ul = document.getElementById('lista-locais-sidebar');
    ul.innerHTML = "";
    
    mapMarkers.forEach(pin => {
        // Verifica se o marker está visível (para busca)
        if (mapInstance.hasLayer(pin.marker)) {
            const li = document.createElement('li');
            li.className = 'sidebar-list-item';
            li.innerHTML = `<strong>${pin.titulo}</strong>`;
            li.onclick = () => {
                mapInstance.flyTo([pin.lat, pin.lng], 2); // Zoom e Pan suave
...
function filtrarPinsMap() {
    const termo = document.getElementById('map-search-input').value.toLowerCase();
    const tipo = document.querySelector('input[name="searchType"]:checked').value; // 'nome' ou 'desc'

    mapMarkers.forEach(pin => {
        let textoAlvo = (tipo === 'nome') ? pin.titulo : (pin.desc || "");
        
        if (textoAlvo.toLowerCase().includes(termo)) {
            if (!mapInstance.hasLayer(pin.marker)) pin.marker.addTo(mapInstance);
        } else {
            if (mapInstance.hasLayer(pin.marker)) mapInstance.removeLayer(pin.marker);
        }
    });
...
// 6. FERRAMENTA DE RÉGUA
function ativarRegua() {
    isMeasuring = true;
    measurePoints = [];
    if (measureLine) mapInstance.removeLayer(measureLine);
    document.getElementById('btn-regua').classList.add('active');
    document.getElementById('medida-result').style.display = 'block';
    document.getElementById('medida-result').innerText = "Clique no ponto A...";
    document.getElementById('leaflet-map-container').style.cursor = 'crosshair';
}
...
function desativarRegua() {
    ...
    measurePoints = [];
    if (measureLine) mapInstance.removeLayer(measureLine);
    document.getElementById('btn-regua').classList.remove('active');
    document.getElementById('medida-result').style.display = 'none';
    document.getElementById('leaflet-map-container').style.cursor = 'default';
}

function processarMedicao(latlng) {
    measurePoints.push(latlng);

    if (measurePoints.length === 1) {
        document.getElementById('medida-result').innerText = "Clique no ponto B...";
    } else if (measurePoints.length === 2) {
        // Desenha linha
        measureLine = L.polyline(measurePoints, {color: 'yellow', dashArray: '5, 10'}).addTo(mapInstance);
        
        // Calcula distância (Pixel Distance no CRS.Simple)
        const p1 = mapInstance.latLngToLayerPoint(measurePoints[0]);
        const p2 = mapInstance.latLngToLayerPoint(measurePoints[1]);
        const distPixels = p1.distanceTo(p2);
        
        // CONVERSÃO (Arbitrária: Exemplo 50 pixels = 1.5 metros)
        // Ajuste o fator 0.03 conforme o tamanho do seu mapa
        const distMetros = (distPixels * 0.03).toFixed(1);

        document.getElementById('medida-result').innerText = `Distância: ~${distMetros}m`;
        
        // Reseta para medir de novo a partir do segundo ponto ou limpa?
        // Vamos limpar para permitir nova medição limpa no próximo clique
        setTimeout(() => { 
             // Opcional: manter a linha por uns segundos
        }, 2000);
        
        // Prepara para próxima (clique 3 vira novo ponto A)
        measurePoints = [];
    }
}