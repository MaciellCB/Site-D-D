...
/* ---------------- MODAL MAGIA (COM CONCENTRAÇÃO, RITUAL E RESISTÊNCIA) ---------------- */
function openSpellModal(existingSpell = null) {
  ...
  // Verifica estados das tags
  const isConc = existingSpell && existingSpell.concentration ? 'checked' : '';
  const isRit = existingSpell && existingSpell.ritual ? 'checked' : '';
          ...
          <div style="flex-shrink:0;">
              <div style="display: flex; gap: 10px; align-items: flex-end;">
                  ...
                  <div style="display: flex; gap: 8px; padding-bottom: 6px;">
                      <label class="check-inline-tag"><input id="modal-conc" type="checkbox" ${isConc} /> <span class="tag-box conc">Concentração</span></label>
                      <label class="check-inline-tag"><input id="modal-rit" type="checkbox" ${isRit} /> <span class="tag-box rit">Ritual</span></label>
                  </div>
              </div>
              ...
              <div class="modal-row" style="margin-top:10px;">
                ...
                <div><label>Execução</label><input id="modal-exec" type="text" value="${existingSpell && existingSpell.attrs && existingSpell.attrs.execucao ? escapeHtml(existingSpell.attrs.execucao) : '1 ação'}" /></div>
                <div><label>Alcance</label><input id="modal-alc" type="text" value="${existingSpell && existingSpell.attrs && existingSpell.attrs.alcance ? escapeHtml(existingSpell.attrs.alcance) : ''}" /></div>
              </div>
              ...
              <div class="modal-row">
                ...
                <div><label>Resistência (Salva)</label><input id="modal-res" type="text" value="${existingSpell && existingSpell.attrs && existingSpell.attrs.resistencia ? escapeHtml(existingSpell.attrs.resistencia) : ''}" placeholder="Ex: DEX" /></div>
              </div>
              ...
              <div class="modal-row">
                ...
                <div style="flex:1"><label>Material</label><input id="modal-material" type="text" value="${existingSpell && existingSpell.material ? escapeHtml(existingSpell.material) : ''}" /></div>
                <div style="flex:1"><label>Damage / Cura (Ex: 1d8)</label><input id="modal-damage" type="text" value="${existingSpell && existingSpell.damage ? escapeHtml(existingSpell.damage) : ''}" /></div>
              </div>
          </div>
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', (ev) => {
      ...
      const novo = {
        ...
        // --- NOVAS TAGS ---
        concentration: modal.querySelector('#modal-conc').checked,
        ritual: modal.querySelector('#modal-rit').checked,
        ...
        attrs: {
          ...
          resistencia: modal.querySelector('#modal-res').value.trim() // Salva Resistência
        },
...
/* --- SUBSTITUA A FUNÇÃO openSpellCatalogOverlay INTEIRA POR ESTA --- */
function openSpellCatalogOverlay(parentModal = null) {
  ...
  const renderCatalogPills = (list, type) => {
    if (list.length === 0) return '<em style="color:#666; font-size:12px;">Nenhuma disponível</em>';
    return list.map(item => `
          <button type="button" class="filter-pill catalog-filter-pill" data-type="${type}" data-val="${escapeHtml(item)}">
              ${item}
          </button>
      `).join('');
  };
  ...
  const circlesHtml = filters.map((f, idx) => {
    return `<button type="button" class="circle-filter ${idx === 0 ? 'active' : ''}" data-filter="${f.val}">${f.label}</button>`;
  }).join('');
  ...
  overlay.innerHTML = `
      <div class="catalog-large" role="dialog" aria-modal="true">
        ...
        <div id="painelFiltrosCatalog" style="display:none; flex-shrink: 0; background:#1a1a1a; padding:15px; border-radius:6px; border:1px solid rgba(156, 39, 176, 0.3); margin-top:10px; margin-bottom:5px;">
           ...
           <div style="margin-bottom: 8px; margin-top: 15px;"><strong style="color:#9c27b0; font-size:11px; text-transform:uppercase;">Filtrar por Classe</strong></div>
           <div class="filter-container-scroll" id="catFiltrosClasse" style="margin-bottom:0;">
...
diceStyles.textContent = `
    ...
    .inspiration-icon {
        ...
        text-shadow: 0 0 15px rgba(255, 215, 0, 0.8), 0 0 5px #fff; cursor: help;
    }

    .dice-content-wrapper {
        flex-grow: 1; padding: 15px; padding-right: 25px;
    ...
    .dice-value-wrapper {
        position: relative; text-align: right; max-width: 220px;
    }
    ...
    .dice-value-wrapper:active .dice-tooltip,
    .dice-value-wrapper:focus-within .dice-tooltip {
    ...
    .crit-total { color: #e040fb; text-shadow: 0 0 10px rgba(224, 64, 251, 0.8); }
    .fumble-total { color: #ff3333; text-shadow: 0 0 10px rgba(255, 51, 51, 0.8); }
`;
...
document.addEventListener('click', function (e) {
  ...
  if (e.target.classList.contains('dice-img')) {
    ...
    if (spellCard) {
      ...
      // =========================================================
      // LÓGICA DE MULTI-RAIOS (Rajada Mística / Raio Ardente)
      // =========================================================
      let numRaios = 0;
      ...
      if (nomeLimpo.includes("rajada mística") || nomeLimpo.includes("eldritch blast")) {
        const charLevel = getTotalCharacterLevel();
        numRaios = 1;
        if (charLevel >= 5) numRaios = 2;
        if (charLevel >= 11) numRaios = 3;
      ...
      if (damageFormula.includes('(')) damageFormula = damageFormula.split('(')[0].trim();
      ...
      if (damageFormula && damageFormula !== '-' && damageFormula !== '0') {
        if (attackRes && attackRes.isCrit) {
          ...
          if (match) {
            ...
            const faces = match[2];
            const resto = match[3] || "";
            damageFormula = `${qtd * critMult}d${faces}${resto}`;
          }
        }
      ...
      if (item) {
        ...
        if (expressionDano && expressionDano !== '-' && expressionDano !== '0') {
          if (attackRes && attackRes.isCrit) {
            ...
            if (match) {
              let qtdDados = match[1] === "" ? 1 : parseInt(match[1]);
              const faces = match[2];
              const restoExpressao = match[3] || "";
              const novaQtd = qtdDados * fator;
  ...
  return `
        <div class="header-dice-wrapper" onclick="event.stopPropagation();">
            ...
            <div class="raw-dice-drawer">
                <div class="raw-dice-btn" data-dice="1d2" title="Rolar 1d2"><span>d2</span></div>
                <div class="raw-dice-btn" data-dice="1d4" title="Rolar 1d4"><span>d4</span></div>
                <div class="raw-dice-btn" data-dice="1d6" title="Rolar 1d6"><span>d6</span></div>
                <div class="raw-dice-btn" data-dice="1d8" title="Rolar 1d8"><span>d8</span></div>
                <div class="raw-dice-btn" data-dice="1d10" title="Rolar 1d10"><span>d10</span></div>
                <div class="raw-dice-btn" data-dice="1d12" title="Rolar 1d12"><span>d12</span></div>
                <div class="raw-dice-btn" data-dice="1d20" title="Rolar 1d20"><span>d20</span></div>
            </div>
        </div>
    `;
}
