        ...
        async function openAdminAccounts() {
            ...
            async function createAdminLocal() {
                ...
                try {
                    ...
                    if (res.ok) {
                        mostrarAviso('Conta criada. Fazendo login...');
                        document.getElementById('admin-create-username').value=''; document.getElementById('admin-create-password').value='';
                        // attempt automatic login
                        try {
                            const r2 = await fetch(`${API_URL}/accounts/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, password }) });
                            if (r2.ok) { const data = await r2.json(); localStorage.setItem('authToken', data.token); mostrarAviso('Logado como ' + username); loadAccountsRefresh(); }
                            else { loadAccountsRefresh(); }
                        } catch(e) { loadAccountsRefresh(); }
                    } else { const d = await res.json().catch(()=>({})); mostrarAviso('Erro: '+(d.error||'Não foi possível criar.')); }
                } catch (e) { mostrarAviso('Erro de conexão.'); }
            }
