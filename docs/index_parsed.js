    ...
    window.addEventListener('DOMContentLoaded', async () => {
      // Try to auto-login using token stored in localStorage.
      const API_BASE = 'https://dandd-chan.onrender.com/api';
      try {
        const token = localStorage.getItem('authToken');
        if (token) {
          const meRes = await fetch(`${API_BASE}/accounts/me`, { headers: { 'Authorization': 'Bearer ' + token } });
          if (meRes.ok) {
            const acc = await meRes.json();
            window.currentAccount = acc;
            if (acc.characters && acc.characters.length > 0) {
              const first = acc.characters[0];
              const res = await fetch(`${API_BASE}/load-ficha-account`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token }, body: JSON.stringify({ nome: first }) });
              if (res.ok) {
      ...
      // If no token or auto-login failed, show login overlay normally.
    });
    ...
    async function tentarLogin() {
      ...
      try {
        ...
        const me = await fetch(`${API_BASE}/accounts/me`, { headers: { 'Authorization': 'Bearer ' + data.token } });
        if (!me.ok) {
          if (masterWindow) masterWindow.close();
    ...
    function showAccountCharacters(acc) {
      ...
      chars.forEach(n => {
        ...
        const img = document.createElement('img'); img.src = 'img/imagem-no-site/personagem.png';
        ...
        const editBtn = document.createElement('button'); editBtn.className = 'btn-edit-account'; editBtn.textContent = 'Ver / Editar (Mestre)'; editBtn.onclick = () => window.open(`mestre.html?open=${encodeURIComponent(n)}`,'_blank');
    ...
    async function criarPersonagem(nome) {
      ...
      try {
        const res = await fetchWithAuth(`${API_BASE}/criar-ficha`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nome, accountUsername: window.currentAccount.username }) });
        if (res.ok) {
          // refresh account
          const me = await fetchWithAuth(`${API_BASE}/accounts/me`);
          if (me.ok) {
